...
{SETTINGS_TEMPLATE}""")
    ...
    _root_directory = config.get('PROJECt_INPUTS', 'root_directory', fallback=os.getcwd())
    ...
    _files: list[str] = [] if "," not in _files_in else [item.strip() for item in _files_in.split(',')]
    _typicals: list[str] = [] if not "," in _typicals_in else [item.strip() for item in _typicals_in.split(',')]
    _filepaths: list[str] = [] if not "," in _filepaths_in else [item.strip() for item in _filepaths_in.split(',')]
